...
const FOOTER_MENU_POSITIONS = new Set([
  'footer_column_1',
  'footer_column_2',
  'footer_column_3',
]);
...
export class MenuService {
  ...
  async addItem(menuId: number, dto: CreateMenuItemDto) {
    const menu = await this.menuRepo.findOne({ where: { id: menuId } });
    if (!menu) throw new NotFoundException('Menu không tồn tại');
    if (FOOTER_MENU_POSITIONS.has(menu.position)) {
      const itemCount = await this.itemRepo.count({ where: { menuId } });
      if (itemCount >= FOOTER_MENU_ITEM_LIMIT) {
        throw new BadRequestException(
          `Mỗi cột footer chỉ được tối đa ${FOOTER_MENU_ITEM_LIMIT} liên kết`,
        );
      }
    }
    const saved = await this.itemRepo.save(
      this.itemRepo.create({ ...dto, menuId }),
    );
    this.auditLogsService.log({
      entityType: 'MenuItem',
      ...
      after: JSON.stringify({
        ...
        openInNewTab: saved.openInNewTab,
      }),
    });
